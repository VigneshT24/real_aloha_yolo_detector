#!/usr/bin/env python3
"""
YOLOv12 + Intel RealSense — real-time object detection with depth
...
class ObjectMemory:
    def __init__(self, hold_seconds=2.0, move_threshold=30):
        self.hold_seconds = hold_seconds
        self.move_threshold = move_threshold  # pixels
        self.memory = {}  # label -> {cx, cy, depth, first_seen, locked}
    ...
    def check_depth_changed(self, depth_image, label, threshold=0.05):
        """Check if depth at remembered location has changed significantly."""
        ...
        if current_depth == 0:
            return False  # no depth reading
        
        # if depth changed significantly, object moved
    ...
    def cleanup_stale(self, timeout_seconds=5.0):
        ...
        for key, val in self.memory.items():
            if 'last_seen' in val and (now - val['last_seen']) > timeout_seconds:
                to_delete.append(key)
        for key in to_delete:
            del self.memory[key]

def get_depth_at(depth_frame, x, y, sample=5):
    """Average depth over a small patch to reduce noise (in metres)."""
    depths = []
...
def main():
    ...
    # Depth scale: converts raw units → metres
        ...
        while True:
            ...
            if ids is not None:
                ...
                for i in range(len(ids)):
                    ...
                    state = obj_memory.update(f"aruco_{mid}", cx, cy, 0.0)
            ...
            for lbl, state in obj_memory.get_locked().items():
                if lbl not in locked_items:
                    mcx, mcy = state['cx'], state['cy']
                    cv2.circle(color_image, (mcx, mcy), 10, (255, 100, 0), 2)
                    cv2.putText(color_image, f"[MEM] {lbl} {state['depth']:.2f}m",
                    (mcx - 40, mcy - 15),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 100, 0), 2)

            depth_array = np.asanyarray(depth_frame.get_data())

            depth_clipped = np.clip(depth_array, 0, 4000)  # max 4 meters
            ...
            cv2.putText(display, f"Objects: {len(results.boxes)}",
